...
void*
xrealloc(void* to_realloc, size_t tgt_sz)
{
    void* res = xmalloc(tgt_sz);
    unsigned long ptr = (unsigned long)to_realloc;
    ptr -= sizeof(long);
    size_t old_size = *((long*)ptr);
    memmove(res, to_realloc, min(old_sz, tgt_sz));
    xfree(to_realloc);
    return res;
}
